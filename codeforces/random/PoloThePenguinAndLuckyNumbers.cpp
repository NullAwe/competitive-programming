...
int main() {
  ...
  // Mods powers of 10:
  ll p10[n + 1], p2[n + 1]; p10[0] = 1, p2[0] = 1;
  for (int i = 1; i <= n; ++i) p10[i] = p10[i - 1] * 10 % MOD, p2[i] = p2[i - 1] * 2 % MOD;
  shared = shared % MOD;
  // Mods start prefixes with 7 replacing the last number:
  ll bm[n + 1], cur = shared; bm[0] = 0, bm[1] = (shared * p10[n] + 4 * p10[n - 1]) % MOD;
  for (int i = 2; i <= n; ++i) {
    cur = (cur * 10 + begin[i - 2] - '0') % MOD;
    bm[i] = ((cur * 10 + 7) % MOD) * p10[n - i] % MOD;
  }
  // Mods end prefixes with 4 replacing the last number:
  ll em[n + 1]; em[0] = 0, em[1] = (shared * p10[n] + 4 * p10[n - 1]) % MOD; cur = shared;
  for (int i = 2; i <= n; ++i) {
    cur = (cur * 10 + end[i - 2] - '0') % MOD;
    em[i] = ((cur * 10 + 4) % MOD) * p10[n - i] % MOD;
  }
  ...
  for (int i = 2; i <= n; ++i) {
    ll f = 4 * p10[i - 1] % MOD, s = 7 * p10[i - 1] % MOD, num = p2[i - 1];
  ...
  ll ans = dpm[n - ind - 1] + ((dps[n - ind - 1] * 2 - dp4[n - ind - 1] - dp7[n - ind - 1] + 2 * MOD) % MOD) * bm[ind + 1] % MOD + (bm[ind + 1] * bm[ind + 1] % MOD) * (p2[n - ind - 1] + MOD - 1) % MOD;
  ans %= MOD;
  ...
  while (ind > 0) {
    ...
    ans %= MOD;
    ans += dpm[n - ind - 1] + ((dps[n - ind - 1] * 2 - dp4[n - ind - 1] - dp7[n - ind - 1] + 2 * MOD) % MOD) * bm[ind + 1] % MOD + (bm[ind + 1] * bm[ind + 1] % MOD) * (p2[n - ind - 1] + MOD - 1) % MOD;
    ans %= MOD;
  ...
  while (ind < n) {
    ...
    ans %= MOD;
    ans += dpm[n - ind - 1] + ((dps[n - ind - 1] * 2 - dp4[n - ind - 1] - dp7[n - ind - 1] + 2 * MOD) % MOD) * em[ind + 1] % MOD + (em[ind + 1] * em[ind + 1] % MOD) * (p2[n - ind - 1] + MOD - 1) % MOD;
    ans %= MOD;
